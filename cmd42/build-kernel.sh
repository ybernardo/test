...
CMD42_SOURCE_DEB="$(find . -maxdepth 1 -type f -name 'linux-source-6.18_*.deb' -print -quit)"
CMD42_IMAGE_DEB="$(find . -maxdepth 1 -type f -name 'linux-image-6.18.39+rpt-rpi-v8_*.deb' -print -quit)"
...
sha256sum kernel8-cmd42-* config-* expose-locked-sd.patch > SHA256SUMS
